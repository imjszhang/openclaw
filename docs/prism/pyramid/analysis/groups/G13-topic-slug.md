...
| MV-03 | merge-main-upgrade-summary-v2 | fork 与 main 实际代码差异收窄为 15 文件 949 行，Telegram HTML 和 Moonshot Kimi 被上游吸收   |

## 组内逻辑顺序

按照"架构设计原则 → 分支管理与同步流程 → 环境适配与故障修复（Windows/路径）→ 定制化分层策略 → 运维与升级规范 → 差异收窄趋势验证（MV-03）"的结构顺序排列。
