...
_注：本次更新中，S28-S30 获得双 Group 支撑，升级为正式候选。当前无待成熟候选。_
...
- **S9 (核心架构) → S4, S5, S6, S7, S8, S10...S30**：支撑关系。S9 定义的"本地优先网关 + 五层抽象"是整个系统的基石，S4(安全)、S5(工程)、S6(Fork)、S7(渠道)、S8(Agent) 等均是在此架构上的具体实践或子系统。
- **S4 ∥ S5 ∥ S10 ∥ S21 ∥ S27 ∥ S28 ∥ S30**：并列关系。共同构成系统质量与安全保障体系：S4 关注运行时纵深防御，S5 关注代码库工程效率，S10 关注配置与状态健康，S21 关注扩展生态的安全加载规范，S27 关注执行命令的动态审批，S28 关注文件系统权限合规，S30 关注全局配置开放边界。
...
- **整体结构**：
  ...
  - **保障层**：S4 (安全), S5 (工程), S6 (版本), S10 (健康), S21 (扩展安全), S26 (路径配置), S27 (执行审批), S28 (权限合规), S30 (配置边界)
  - **功能层**：S7 (渠道), S8 (Agent), S14 (浏览器中继), S15 (调度), S16 (终端), S23 (浏览器复用), S29 (记忆治理)
  - **扩展层**：S17 (插件框架), S18 (集成), S19 (实例), S20 (技能), S24 (分发), S25 (发现)
