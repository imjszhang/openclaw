...
_注：本次更新中，S34 基于新提取的 G53 及已有 CB groups 支撑，直接升级为正式候选。当前无待成熟候选。_
...
- **S9 (核心架构) → S4, S5, S6, S7, S8, S10...S34**：支撑关系。S9 定义的"本地优先网关 + 五层抽象"是整个系统的基石，S4(安全)、S5(工程)、S6(Fork)、S7(渠道)、S8(Agent) 等均是在此架构上的具体实践或子系统。
...
- **整体结构**：
  ...
  - **扩展层**：S17 (插件框架), S18 (集成), S19 (实例), S20 (技能), S24 (分发), S25 (发现), S32 (生态导航)
