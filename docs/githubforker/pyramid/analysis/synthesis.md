...
_注：本次更新中，原 S6\* 已获得 G13 支撑升级为正式候选 S6；原 S7\*-S20\* 均获得第二组支撑（主要来自新提取的 detailed guides），全部升级为正式候选。当前无待成熟候选。_
...
- **S9 (核心架构) → S4, S5, S6, S7, S8, S10...S20**：支撑关系。S9 定义的"本地优先网关 + 五层抽象"是整个系统的基石，S4(安全)、S5(工程)、S6(Fork)、S7(渠道)、S8(Agent) 等均是在此架构上的具体实践或子系统。
- **S4 ∥ S5 ∥ S10**：并列关系。三者共同构成系统质量保障体系：S4 关注运行时安全，S5 关注代码库工程效率，S10 关注配置与状态健康。
...
- **S17 ∥ S18 ∥ S19 ∥ S20**：扩展生态并列。分别关注插件开发、外部脚本集成、Agent 实例管理、技能加载机制，共同构成系统的可扩展性。
- **整体结构**：
  - **核心层**：S1-S3 (知识方法论), S9 (系统架构)
  - **保障层**：S4 (安全), S5 (工程), S6 (版本), S10 (健康)
  - **功能层**：S7 (渠道), S8 (Agent), S14 (浏览器), S15 (调度), S16 (终端)
  - **扩展层**：S17 (插件), S18 (集成), S19 (实例), S20 (技能)
